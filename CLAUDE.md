...
`POST /api/apps/:app/archive` snapshots the full app directory to `.ralph-flow/.archives/<appName>/<YYYY-MM-DD_HH-mm>/`, then resets the app in place: tracker files revert to template state, data files (stories.md, tasks.md) reset to headers only, directories listed in loop `directories` config are emptied (contents removed, directory preserved), `.agents/` dirs and lock files are cleaned up, and SQLite `loop_state` rows are deleted. Prompt files and `ralphflow.yaml` are preserved. Uses `resolveTemplatePathWithCustom(config.name, cwd)` to find template originals for reset. When the template cannot be resolved (deleted, renamed, or custom template removed), tracker and data files fall back to minimal empty content instead of skipping the reset — a warning is logged and included in the API response. Same-minute collisions append a sequence suffix (e.g., `2026-03-14_15-30-2`). The `.archives/` directory is dotfile-prefixed so `listFlows()` ignores it.
...
### Archive Viewer UI (ui/archives.js)
The app detail view has a "Loops" / "Archives" tab bar below the app header. The "Loops" tab shows the existing pipeline, commands, and three-panel loop layout. The "Archives" tab fetches snapshots from `GET /api/apps/:app/archives` and renders them as expandable timeline cards (newest-first) showing date, story count, task count, and file count. Clicking a card loads its file listing; clicking a file displays its content inline in a read-only monospace viewer. Empty state shown when no archives exist. Tab selection (`activeAppTab`) persists when switching loops within the same app; resets to "Loops" on app change. When `summary.md` exists in an archive, it is auto-fetched on card expand and rendered as markdown (via `renderMarkdown()`) in a `.archive-summary` block at the top of the card — above the file list. The file is filtered from the regular file list to avoid duplication. Archives without `summary.md` show a subtle hint (`.archive-summary-hint`) with the pre-filled CLI command `npx ralphflow summarize <app> <timestamp>`, using dimmed text and a copyable monospace code block (`user-select: all`). The hint is replaced by the actual summary after generation and re-expand. Summary content is cached in `state.archiveSummaryCache[timestamp]`.

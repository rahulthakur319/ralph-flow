// Server-side prompt generation engine.
// Ported from dashboard/ui/prompt-builder.js for CLI use.
...
function defaultCapabilities(): StageConfig['capabilities'] {
  return {
    webSearch: false,
    mcpServers: false,
    exploreAgents: false,
...
/**
 * Build a PromptLoopConfig from a TemplateLoopDefinition-like object.
 * Applies sensible defaults for stage configs when not provided.
 */
export function buildPromptLoopConfig(loopDef: {
  name: string;
  stages: string[];
  completion: string;
  multi_agent?: { enabled: boolean } | false;
  entities?: string[];
  feeds?: string[];
  fed_by?: string[];
}): PromptLoopConfig {
  const stages = loopDef.stages;
  const stageConfigs: StageConfig[] = stages.map((stageName, i) => {
    const caps = defaultCapabilities();
    // Apply sensible defaults based on stage position
    if (i === 0) {
      // First stage: read files, explore
      caps.fileReadWrite = true;
      caps.exploreAgents = true;
    }
    if (i === stages.length - 1 && stages.length > 1) {
      // Last stage: verify via bash
      caps.bashCommands = true;
      caps.fileReadWrite = true;
    }
    // Middle stages: code editing + bash
    if (i > 0 && i < stages.length - 1) {
      caps.codeEditing = true;
      caps.bashCommands = true;
      caps.fileReadWrite = true;
    }
    // If only one stage, enable all common capabilities
    if (stages.length === 1) {
      caps.codeEditing = true;
      caps.bashCommands = true;
    }
...
/**
 * Generate a complete structured prompt from a loop configuration.
 * Server-side equivalent of the client-side generatePromptFromConfig().
 */
export function generatePromptFromConfig(loop: PromptLoopConfig, loopIndex: number, allLoops: PromptLoopConfig[]): string {
